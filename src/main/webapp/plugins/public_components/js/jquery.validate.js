    ...
    $.extend($.validator, {
        ...
        defaults: {
            ...
            ignore: function(i,dom){
                var r_dom ="";
                if( !$(dom).hasClass("select2_input") && ( $(dom)[0].type != "radio" && $(dom)[0].type !="checkbox" ) && $(dom).is(":hidden")   ){
                    r_dom = dom;
                }
            ...
            onselect2selected : function(element, event){
                this.element(element);
            },
        ...
        prototype: {
            ...
            init: function() {
                ...
                $(this.currentForm)
                    ...
                    .validateDelegate("input.select2_input","select2-selected", delegate)
            ...
            elements: function() {
                ...
                return $(this.currentForm)
                    .find("input, select, textarea, [type='text']")
            ...
            showLabel: function(element, message) {
                ...
                if ( label.length ) {
                    ...
                    if ( !this.labelContainer.append(label).length ){
                        if(element.type =="radio"){
                            $(element).parent("label").parent().append(label);
...
jQuery.validator.addMethod("compareTime",
    function(b, a, c) {
        var d = c[1] || ">",
            e = c[2] || "yyyy-MM-dd";
...
jQuery.validator.addMethod("compareTo",
    function(b, a, c) {
        a = $(c[0]);
        c = c[1];
        if ($.trim(b) !== "" && $.trim(a.val()) !== "") try {
            return (new Function("return " + (b + c + a.val()) + ";"))()
        } catch(d) {
            return ! 1
        } else return ! 0
    },
...
jQuery.validator.addMethod("regExp",
    ...
    "\u8f93\u5165\u7684\u4fe1\u606f\u4e0d\u5339\u914d\u5b9a\u4e49\u7684\u89c4\u5219\uff01");
jQuery.validator.addMethod("CNRangeLength",
    function(b, a, c) {
...
jQuery.validator.addMethod("IP",
    ...
    "\u8bf7\u8f93\u5165\u6b63\u786e\u7684IP\u5730\u5740\uff01");
jQuery.validator.addMethod("MAC",
    function(b, a) {
...
jQuery.validator.addMethod("mobile",
    ...
    "\u8bf7\u8f93\u5165\u6b63\u786e\u7684\u624b\u673a\u53f7\u7801\uff01");
jQuery.validator.addMethod("dateCN",
    function(b, a) {
...
jQuery.validator.addMethod("chinese",
    ...
    "\u8bf7\u8f93\u5165\u4e2d\u6587\u5b57\u7b26\uff01");
jQuery.validator.addMethod("validChar",
    function(b, a) {
...
function _checkIDCard_(b) {
    ...
    if (/(^\d{15}$)|(^\d{17}([0-9]|X|x)$)/.test(b)) if (c[b.substr(0, 2)] == null || c[b.substring(0, 2)] == "") return a = !1;
    else switch (b.length) {
            case 15:
                if (d(parseInt(b.substr(6, 2), 10) + 1900 + "-" + b.substr(8, 2) + "-" + b.substr(10, 2))) a = !0;
                else return a = !1;
                break;
            case 18:
                if (!d(b.substr(6, 4) + "-" + b.substr(10, 2) + "-" + b.substr(12, 2))) return a = !1;
                b = b.split("");
                if ("10X98765432".substr(((parseInt(b[0], 10) + parseInt(b[10], 10)) * 7 + (parseInt(b[1], 10) + parseInt(b[11], 10)) * 9 + (parseInt(b[2], 10) + parseInt(b[12], 10)) * 10 + (parseInt(b[3], 10) + parseInt(b[13], 10)) * 5 + (parseInt(b[4], 10) + parseInt(b[14], 10)) * 8 + (parseInt(b[5], 10) + parseInt(b[15], 10)) * 4 + (parseInt(b[6], 10) + parseInt(b[16], 10)) * 2 + parseInt(b[7], 10) * 1 + parseInt(b[8], 10) * 6 + parseInt(b[9], 10) * 3) % 11, 1) != b[17]) return a = !1;
                a = !0
        } else return a = !1;
    return a
}
function _compareTime_(b, a, c, d) {
    ...
    Date.prototype.format = function(b) {
        ...
        for (var c in a) RegExp("(" + c + ")").test(b) && (b = b.replace(RegExp.$1, RegExp.$1.length == 1 ? a[c] : ("00" + a[c]).substr(("" + a[c]).length)));
        return b
    }
})();
...
(function(){
    ...
    var addRule = function( options ){
        ...
        this.find("input, select, textarea, [type='text']")
            .not(":submit, :reset, :image, [disabled]").not(":hidden")
            .each(function( i, elm ){
                for(let attr of this.attributes){
                    var fun  =$.validator.methods[attr.name];
                    if( typeof fun == "function" ){
                        _opt.rules[this.id] || (_opt.rules[this.id] = {});
                        _opt.rules[this.id][attr.name] = eval(attr.value) ;
                    }
                }
            })
        this.validate( _opt );
    };
    $.extend($.fn, {"addRule": addRule});
}());		