                ...
                <c:if test="${depotArea.id==null}">
                    <div class="input-append span12 required">
                        <input type="text" class="span11" id="areaCode" name="areaCode" placeholder="库区编码"
                               <c:if test="${depotArea.id==null}">value="${areaCode}" </c:if>/>
                    </div>
                </c:if>
                <c:if test="${depotArea.id!=null}">
                    <div class="input-append span12 required">
...
<script type="text/javascript">
    ...
    $("#materielManagerForm").validate({
        rules: {
            ...
            "areaCode": {
                ...
                maxlength: 16,
                <c:if test="${depotArea.id==null}">
                remote: context_path + "/depotAreaManage/hasC?id=" + $('#id').val()
                </c:if>
            },
            /*库区名称*/
            "areaName": {
    ...
    function saveTaskDetail() {
        ...
        if ($("#materielManagerForm" +
            ...
            $.ajax({
                url: context_path + "/depotAreaManage/addDepotArea?tm=" + new Date(),
                type: "POST",
                data: roleParam,
                dataType: "JSON",
