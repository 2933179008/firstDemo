...
<div id="grid-div">
    <form id="hiddenForm" action="<%=path%>/customerManage/toExcel" method="POST" style="display: none;">
        <input id="ids" name="ids" value=""/>
    </form>
    <form id="hiddenQueryForm" style="display:none;">
        <input id="customerCodeS" name="customerCode" value=""/>
        <input id="customerNameS" name="customerName" value="">
    </form>
    <div class="query_box" id="yy" title="查询选项">
        <form id="queryForm" style="max-width:100%;">
            <ul class="form-elements">
                <li class="field-group field-fluid3">
                    <label class="inline" for="customerCode" style="margin-right:25px;width:100%;">
                        <span class="form_label" style="width:80px;">客户编码：</span>
                        <input type="text" name="customerCode" id="customerCode" value=""
                               style="width: calc(100% - 120px);" placeholder="客户编码">
                    </label>
                </li>

                <li class="field-group field-fluid3">
                    <label class="inline" for="customerName" style="margin-right:25px;width:100%;">
                        <span class="form_label" style="width:80px;">客户名称：</span>
                        <input type="text" name="customerName" id="customerName" value=""
                               style="width: calc(100% - 120px);" placeholder="客户名称">
                    </label>
                </li>
    ...
    <div class="row-fluid" id="table_toolbar" style="padding:5px 3px;">
        <button class=" btn btn-primary btn-addQx" onclick="addCustomer();">
            添加<i class="fa fa-plus" aria-hidden="true" style="margin-left:5px;"></i>
        </button>
        <button class=" btn btn-primary btn-editQx" onclick="editCustomer();">
            编辑<i class="fa fa-pencil" aria-hidden="true" style="margin-left:5px;"></i>
        </button>
        <button class=" btn btn-primary btn-deleteQx" onclick="deleteCustomer();">
            删除<i class="fa fa-trash" aria-hidden="true" style="margin-left:5px;"></i>
        </button>
        <button class=" btn btn-primary btn-editQx" onclick="lookCustomer();">
            查看<i class="icon-zoom-in" aria-hidden="true" style="margin-left:5px;"></i>
        </button>
        <button class="col-md-1 btn btn-primary btn-queryQx" onclick="exportCustomer();">
...
<script type="text/javascript">
    ...
    $(function () {
        _grid = jQuery("#grid-table").jqGrid({
            url: context_path + "/customerManage/getList",
            datatype: "json",
            colNames: ["主键", "客户编码", "客户名称", "客户类型","联系人", "联系方式", "地址", "邮箱", "备注"],
            colModel: [
                {name: "id", index: "id", width: 20, hidden: true},
                {name: "customerCode", index: "customerCode", width: 65},
                {name: "customerName", index: "customerName", width: 65},
                {name: "customerType", index: "customerType", width: 65},
    ...
    //重置查询条件
    function reset() {
        $("#queryForm #customerCode").val("");
        $("#queryForm #customerName").val("");
        $("#grid-table").jqGrid("setGridParam",
            {
                postData: {queryJsonString: ""} //发送数据
            }
    ...
    function addCustomer() {
        $.post(context_path + "/customerManage/toEdit?id=-1", {}, function (str) {
    ...
    function editCustomer() {
        ...
        if (checkedNum == 0) {
            ...
            $.post(context_path + "/customerManage/toEdit?id=" + id, {}, function (str) {
                $queryWindow = layer.open({
                    title: "客户编辑",
                    type: 1,
                    skin: "layui-layer-molv",
    ...
    function deleteCustomer() {
        ...
        if (checkedNum == 0) {
            ...
            layer.confirm("确定删除选中的客户？", function () {
                $.ajax({
                    type: "POST",
                    url: context_path + "/customerManage/delCustomer?ids=" + ids,
                    dataType: "json",
                    cache: false,
    ...
    function lookCustomer() {
        var checkedNum = getGridCheckedNum("#grid-table", "id");
        if (checkedNum == 0) {
            layer.alert("请选择一个要查看的客户！");
            return false;
        } else if (checkedNum > 1) {
            layer.alert("只能选择一个客户进行查看操作！");
            return false;
        } else {
            var id = jQuery("#grid-table").jqGrid("getGridParam", "selrow");
            $.post(context_path + "/customerManage/toDetail?id=" + id, {}, function (str) {
                $queryWindow = layer.open({
                    title: "顾客查看",
                    type: 1,
                    skin: "layui-layer-molv",
