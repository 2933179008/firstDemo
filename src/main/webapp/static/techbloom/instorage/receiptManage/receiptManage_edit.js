...
$("#formSave").click(function(){
    if($('#baseInfor').valid()){
        $.ajax({
            url:context_path+"/receiptManage/saveReceipt",
            type:"post",
            data:$("#baseInfor").serialize(),
            dataType:"JSON",
            success:function (data){
                if(data.code){
                    $("#baseInfor #id").val(data.receiptId);
                    //刷新收货单列表
                    ...
                    $("#grid-table-c").jqGrid("setGridParam", {
                        url : context_path + '/receiptManage/getDetailList?receiptId='+data.receiptId,
...
$("#formSubmit").click(function(){
    if($("#baseInfor #id").val()!=""){
        layer.confirm("提交后的数据将不能修改，确认提交吗？",function(){
            $.ajax({
                url:context_path+"/receiptManage/submitReceipt?receiptId="+$("#baseInfor #id").val(),
                type:"post",
                dataType:"JSON",
                success:function (data){
                    if(data.result){
...
$("#materialInfor").select2({
    ...
    closeOnSelect:false,
    ajax : {
        url : context_path + "/receiptManage/getSelectMaterial",
        dataType : "json",
        delay : 250,
        data : function(term, pageNo) { //在查询时向服务器端传输的数据
            term = $.trim(term);
...
function addDetail(){
    ...
    if($("#materialInfor").select2("val") != "" && $("#materialInfor").select2("val") != null){
        ...
        $.ajax({
            type:"POST",
            url:context_path + "/receiptManage/saveReceiptDetail",
            data:{
                receiptId:$("#baseInfor #id").val(),
...
function delDetail(){
    ...
    $.ajax({
        url:context_path + '/receiptManage/deleteReceiptDetail?ids='+ids,
        type:"POST",
        dataType:"JSON",
        success:function(data){
            if(data.result){
...
function delOneDetail(id){
    $.ajax({
        url:context_path + '/receiptManage/deleteReceiptDetail?ids='+id,
        type:"POST",
        dataType:"JSON",
        success:function(data){
            if(data.result){
