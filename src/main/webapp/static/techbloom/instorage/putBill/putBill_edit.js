...
$("#baseInfor #instorageBillId").select2({
    ...
    ajax: {
        url: context_path + "/putBill/getSelectInstorage",
        type: "POST",
        dataType: 'json',
        delay: 250,
...
$("#formSave").click(function(){
    if($('#baseInfor').valid()){
        $.ajax({
            url:context_path+"/putBill/savePutBill",
            type:"post",
            data:$("#baseInfor").serialize(),
            dataType:"JSON",
            success:function (data){
                if(data.code){
                    $("#baseInfor #id").val(data.putBillId);
                    //刷新上架单列表
                    ...
                    $("#grid-table-c").jqGrid("setGridParam", {
                        url : context_path + '/putBill/getDetailList?putBIllId='+data.putBillId,
...
$("#materialInfor").select2({
    ...
    closeOnSelect:false,
    ajax : {
        url : context_path + "/putBill/getSelectMaterial",
        dataType : "json",
        delay : 250,
        data : function(term, pageNo) { //在查询时向服务器端传输的数据
            term = $.trim(term);
...
function addDetail(){
    ...
    if($("#materialInfor").select2("val") != "" && $("#materialInfor").select2("val") != null){
        ...
        $.ajax({
            type:"POST",
            url:context_path + "/putBill/savePutBillDetail",
            data:{
                putBillId:$("#baseInfor #id").val(),
...
function delDetail(){
    ...
    $.ajax({
        url:context_path + '/putBill/deletePutBillDetail?ids='+ids,
        type:"POST",
        dataType:"JSON",
        success:function(data){
            if(data.result){
...
function reloadDetailTableList(){
    $("#grid-table-c").jqGrid("setGridParam", {
        postData: {putBillId:$("#baseInfor #id").val()} //发送数据  :选中的节点
    }).trigger("reloadGrid");
}
...
$("#formSubmit").click(function(){
    if($("#baseInfor #id").val()!=""){
        layer.confirm("提交后的数据将不能修改，确认提交吗？",function(){
            $.ajax({
                url:context_path+"/putBill/submitPutBill?putBillId="+$("#baseInfor #id").val(),
                type:"post",
                dataType:"JSON",
                success:function (data){
                    if(data.result){
        ...
        layer.msg("请先保存单据",{icon:2,time:3000})
    }
})
...
function delOneDetail(id){
    $.ajax({
        url:context_path + '/putBill/deletePutBillDetail?ids='+id,
        type:"POST",
        dataType:"JSON",
        success:function(data){
            if(data.result){
