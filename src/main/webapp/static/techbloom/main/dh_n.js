(function(){
	...
	dh_menu.prototype = {
		...
		user_menu : (()=>{return  $.get(context_path + "/user/getUserMenu11");})(),
		afterLoad: function(){
		...
		bindFun : function(){
			...
			$(".all-product input").off("change").on("change", (function(_this){
				return function(){
                	...
                	if($(".seleted-product .selected-item").length < 3){
			...
			$(".dialog-ok-btn").off("click").on("click",function(){
				...
				$.ajax({
					url : context_path + "/user/setUserQuickMenu",
					type:"post",
					data : {menus: arr.join(",")},
					success: function(data){
						if(data == true){
							layer.msg("保存成功！",{icon:1});
							_this.user_menu = (()=>{return  $.get(context_path + "/user/getUserMenu11");})();
                            layer.closeAll();
						}else{
		...
		createDom : function(){
			...
			for(let i = 0; i < menus.length; i++){
				...
				menus_dom +='<div class="service-type"><h3><span style=" padding-bottom: 5px;" >'+menus[i].menuName+'</span></h3><div class="product-list">'+type+'</div></div>';
			...
			return ("<div class='dialog-panel'><div class='left'><h3>导航产品<span style='color:red;'>(最多可以选择3个)</span></h3>"+
