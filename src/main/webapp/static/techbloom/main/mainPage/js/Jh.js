...
Jh.fn = function(a) {
    return a = {
        ...
        _createLi: function(a, c, h ) {
            return $("<li/>").append("<a href='#' rel='" + a + "'>" + (c ? c : a) + "</a>").append("<span class='eye'>"+( h ? "<i class='fa fa-eye'></i>显示": "<i class='fa fa-eye-slash'></i>隐藏" )/*+"</span><span class='remove'><i class='fa fa-trash'></i>删除</span>"*/)
        },
        ...
        _moduleLiClick: function() {
            $("." + Jh.Config.ulCls + " li .eye").live("click",
            function() {
                var a = $(this).parent("li"),
                id = a.find("a").attr("rel"),
                c = $("#" + id+".groupItem");
                c.is(":visible") ? ( $(this).html("<i class='fa fa-eye'></i>显示"), c.hide() ) : ($(this).html("<i class='fa fa-eye-slash'></i>隐藏"), c.show() );
                Jh.Util.refresh();
                if ($.type(Jh.hideUi) == "function"){
                	 Jh.hideUi(id);
                }
            });
            $("." + Jh.Config.ulCls + " li .remove").live("click",
                    function() {
		            	   var a = $(this).parent("li"),
		                   id = a.find("a").attr("rel"),
		                   b = $(".tag-list").find("a[rel='" + id + "']");
		                   $("#"+id).remove();
		                   b.parent().remove();
		                   if ($.type(Jh.removeUi) == "function"){
		                     	 Jh.removeUi(id);
		                     }
                    });
        },
        ...
        _ToLayout: function(a) {
            ...
            function(a, b) {
                ...
                var charts = g.find(".itemContent").children().data("mychart");
            	if(charts){
            		setTimeout(function(){
            			charts.resize();	
            		},0);
...
Jh.Portal = function(a) {
    ...
    d = "<div id='{key}' class='" + Jh.Config._groupItemClass + " {noborder} '/>",
    ...
    return a = {
        ...
        _create: function() {
            a.box = $("<div id='portal' style='display: flex;align-items: stretch;height: 100%;'></div>");
            a._elements = {};
        ...
        _createPortalOne: function($dom, b, c,t ) {
            var e = c ? a._createItemHeader(c) : "",
            f = a._createItemContent($dom);
            return $(Jh.Util.format(d, {
                key: b,
                noborder :  (t == "0" ? "no_border" :"")
            })).append(e).append(f)
        },
...
*/            a._createA(Jh.Config.min, Jh.Config.mintext, !0).appendTo(c);
            ...
            a._createA(Jh.Config.slash, Jh.Config.slashtext, !0).appendTo(c);
        ...
        _createItemContent: function( $dom ) {
        
            var a = $(i);
          /*  a.css("height", $dom.height()+"px");*/
            $dom.appendTo(a);
            return a
        },
        ...
        _eventRlash: function(){
             $("[class='" + Jh.Config.slash+"']").live("click",function(){
            	 var a = $(this),
                 c = a.parents(".groupItem");
                 c.hide();
                 $("[rel="+c.attr("id")+"]").next().html("<i class='fa fa-eye'></i>显示");
                 Jh.Util.refresh();
                 if ($.type(Jh.hideUi) == "function"){
                	 Jh.hideUi(c.attr("id"));
                }
        ...
        _eventSortable: function() {
            ...
            $("." + Jh.Config._groupWrapperClass).sortable({
                ...
                stop: function( event, ui ) {
                	var charts = ui.item.find(".itemContent").children().data("mychart");
                	if(charts){
                		charts.resize();
                	}
                	 if ($.type(Jh.endSort) == "function"){
                    	 Jh.endSort();
                    }
                }
